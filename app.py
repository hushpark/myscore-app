...
if not st.session_state["admin_logged_in"] and not st.session_state["student_logged_in"]:
    with st.container():
        with st.form("master_unified_form"):
            ...
            # 절대 겹칠 수 없는 독립 키 할당으로 원천 봉쇄
            login_mode = st.radio("접속 모드", ["학생", "교사"], horizontal=True, label_visibility="collapsed", key="final_isolated_role_radio")
            
            user_id_input = st.text_input("ID", placeholder="ID를 입력하세요", label_visibility="collapsed", key="final_user_id_input_field")
            user_pw_input = st.text_input("PW", type="password", placeholder="비밀번호를 입력하세요", label_visibility="collapsed", key="final_user_pw_input_field")
            
            b_col1, b_col2, b_col3 = st.columns([1.0, 1.8, 1.0])
            with b_col2:
                # 🔴 [선생님 피드백 반영] '로그인' 문구 수정 완료
...
# =========================================================================
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        menu_selection = st.radio("메뉴 선택", ["▶ 학생 조회 현황 모니터링", "▶ 개인별 성적 입력", "▶ 평가 대상 과목 구성", "▶ 성적 전체 일괄 업로드(CSV)"], label_visibility="collapsed", key="teacher_sidebar_menu_radio")
        st.markdown("---")
        
        st.button("🚪 시스템 로그아웃", type="secondary", use_container_width=True)
    ...
    if menu_selection == "▶ 학생 조회 현황 모니터링":
        with st.container(border=True):
            ...
            if not registered_dbs: st.warning("⚠️ 현재 개설 파티션이 없습니다.")
            else:
        ...
        with st.container(border=True):
            ...
            if not registered_dbs: st.warning("⚠️ 권한 파티션이 없습니다.")
                ...
                if not db_df.empty:
                    ...
                    edited_df = st.data_editor(edit_target_df, use_container_width=True, num_rows="dynamic", disabled=["반", "번호", "이름"], hide_index=True, key="teacher_data_editor_widget")
                    st.markdown("<br>", unsafe_allow_html=True)
        ...
        with st.container(border=True):
            ...
            for i in range(item_count):
                with cols_items[i]:
                    t_in = st.text_input(f"항목 {i+1} 제목", value=f"수행평가_{i+1}", key=f"item_title_input_{i}")
