...
# --- 앱 설정 ---
st.set_page_config(page_title="교과 제어 마스터 센터", layout="wide")
...
if not st.session_state["admin_logged_in"]:
    ...
    with st.form("admin_premium_login_form"):
        st.markdown("<h3 style='text-align:center; font-size:26px; font-weight:700;'>🛡️ 교과 관리자 인증</h3>", unsafe_allow_html=True)
        st.markdown("<p style='text-align:center; font-size:14px; color:#64748b; margin-bottom:25px;'>본인 교과의 성적 데이터를 관리하기 위해<br>인증 비밀번호를 입력해 주세요.</p>", unsafe_allow_html=True)
    ...
    # 상단 네비게이션 바 (컴팩트 와이드 정렬)
    t_col1, t_col2 = st.columns([5.5, 1.5])
