...
# 🚨 [레이아웃 인프라 고정] 최상단 배치 규칙 엄수 - 하얀 본문 바탕 인프라 복구 및 와이드 고정
...
# =========================================================================
# 🔄 전역 테마 통합 스타일 패키지 (사이드바 버그 영역 완벽 숙청 및 본문 카드 이식)
# =========================================================================
st.markdown("""
    <style>
        /* 🚨 [원상복구 완수] 왼쪽 사이드 메뉴판을 침범했던 불필요한 흰색 마스크 박스 레이아웃 전면 폐기 */
        [data-testid="stSidebar"] { background-color: #1e293b !important; }
        
        /* 사이드바 하단 정보수정 및 로그아웃 버튼 오버롤 효과 깔끔 제거 및 기본 고정 */
        ...
        div.stButton > button[key="sidebar_account_btn"]:hover { background-color: #4f46e5 !important; color: #ffffff !important; }
        ...
        div.stButton > button[key="sidebar_logout_btn"]:hover { background-color: #ef4444 !important; color: #ffffff !important; }
        ...
        /* 🚨 [요청 반영] 수행평가 항목 입력 텍스트박스 테두리 상시 선명하게 노출 */
        ...
        /* 🚨 [핵심 복구] 본문 콘텐츠 컴포넌트들을 통째로 예쁘게 가두는 웅장한 대형 흰색 카드 배경 박스 스타일 지정 */
        ...
        /* 하단 정렬 버튼 컬러 스펙트럼 */
        div.stButton > button[key="btn_save_all_grid_changes"] { background-color: #3b82f6 !important; color: white !important; font-weight: bold !important; }
        ...
        div.stButton > button[key="btn_save_evaluation_config"] { background-color: #ff4b4b !important; color: white !important; font-weight: bold !important; }
    </style>
...
if not st.session_state["admin_logged_in"]:
    ...
    # 교사 대시보드 내부 전역 본문 소프트 그레이 연회색 배경 지정
    ...
    # 📊 모듈 1: 학생 조회 현황 모니터링 [🚨 마스터 대형 흰색 카드 박스 복구]
    if menu_selection == "▶ 학생 조회 현황 모니터링":
        # 🚨 [완벽 복구] 본문 콘텐츠 전체를 입체적으로 가두는 마스터 대형 흰색 카드 배경 이식
        st.markdown('<div class="main-white-card-container">', unsafe_allow_html=True)
        ...
        st.markdown('</div>', unsafe_allow_html=True) # 대형 박스 격리 닫기

    # 📝 모듈 2: 개인별 성적 입력 [🚨 마스터 대형 흰색 카드 박스 복구]
    elif menu_selection == "▶ 개인별 성적 입력":
        # 🚨 [완벽 복구] 콘텐츠 영역 전체를 예쁘게 감싸는 마스터 대형 흰색 카드 배경 이식
        st.markdown('<div class="main-white-card-container">', unsafe_allow_html=True)
    ...
    # 📁 모듈 3: 평가 대상 과목 구성 [🚨 마스터 대형 흰색 카드 박스 복구]
    ...
    # 📤 모듈 4: 성적 전체 일괄 업로드(CSV) [🚨 마스터 대형 흰색 카드 박스 복구]
