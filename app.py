...
# =========================================================================
# 🔄 [우주 최강 구역별 물리적 격리 CSS] 로그인 폼 완벽 교정
# =========================================================================
st.markdown("""
    <style>
        ...
        /* 입력창 디자인 통합 및 비밀번호 눈알 버튼 흰색 잔상 제거 */
        ...
        .footer-text { text-align: center; font-size: 11px; color: #94a3b8; margin-top: 20px; }
    ...
    try:
        ...
        for wks in sh.worksheets():
            ...
            if name.startswith("cfg_"):
                ...
                if match: active_list.append({"subject": match.group(1).replace("_", " "), "grade": f"{match.group(2)}학년", "semester": match.group(3).replace("_", " ")})
    except: pass
    return active_list
        ...
        if st.form_submit_button("시스템 로그인"):
            if login_mode == "교사":
                ...
                if user_id_input and user_pw_input:
                    ...
                    st.rerun()
                else: st.error("❌ 학생 ID와 비밀번호를 모두 입력해 주세요.")

    st.markdown("<div class='footer-text'>Designed & Developed by User & AI Creator</div>", unsafe_allow_html=True)

# =========================================================================
# 🎓 [2단계-A] 학생 전용 대시보드 (로그인 후 과목 선택)
# =========================================================================
elif st.session_state["student_logged_in"]:
    st.markdown(f"<h2>수행평가 점수 확인 시스템 <span style='font-size:18px; color:#3b82f6;'>(학생 모드)</span></h2>", unsafe_allow_html=True)
    st.write(f"👤 접속 중인 ID: **{st.session_state['logged_student_id']}**")
    
    if st.button("🚪 시스템 로그아웃", type="secondary"):
        st.session_state["student_logged_in"] = False
        st.session_state["logged_student_id"] = ""
        st.session_state["logged_student_pw"] = ""
        st.rerun()
        
    st.markdown("---")
    st.markdown("### 📚 과목별 성적 조회")
    ...
    if not active_dbs:
        ...
        if sel_s != "과목 및 학기를 선택하세요.":
            ...
            if st.button("나의 성적 조회", type="primary"):
                ...
                if config:
                    ...
                    if not df_st.empty:
                        ...
                        if not res.empty:
                            ...
                            st.error("❌ 해당 과목 시트에 정보가 없거나, 비밀번호가 일치하지 않습니다.")
        ...
        with st.container(border=True):
            ...
            if not registered_dbs:
                ...
                with col_class:
                    ...
                    selected_class = st.selectbox("🎯 필터링할 학급(반) 선택", options=class_options)
            ...
            if not registered_dbs:
                ...
                if not db_df.empty:
                    ...
                    with btn_col3:
                        st.write("")
                    with btn_col1:
                        if st.button("➕ 학생 개별 추가", use_container_width=True):
                            student_individual_add_dialog(db_df, sf_id, score_headers)
                    with btn_col2:
                        if st.button("💾 수정 사항 저장", use_container_width=True):
    ...
    # 📁 모듈 3: 평가 대상 과목 구성
        ...
        with st.container(border=True):
            ...
            for i in range(item_count):
                with cols_items[i]:
                    t_in = st.text_input(f"항목 {i+1} 제목", placeholder="수행평가 항목 입력", label_visibility="collapsed")
